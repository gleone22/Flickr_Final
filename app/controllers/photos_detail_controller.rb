class PhotosDetailController < ApplicationController
  def index
    ...
    @photo_detail << response['photo']['secret']
    @photo_detail << response['photo']['server']
    @photo_detail << response['photo']['farm']
    @photo_detail << response['photo']['owner']['nsid']
# this is the descript./ tags
@photo_desc = ""
@photo_desc <<  @photo_detail[5]
end
